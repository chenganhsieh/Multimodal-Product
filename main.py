...
# import datasets
# import hooks
# from ocrclip import dynamic_multi_gpu_test, dynamic_single_gpu_test
...
def setup_multi_processes(cfg):
    ...
    if platform.system() != 'Windows':
        mp_start_method = cfg.get('mp_start_method', 'fork') #  temp comment for taiji runtime metric
...
def main():
    ...
    if args.work_dir is not None:
        ...
        cfg.work_dir = osp.join(args.work_dir,
                 f'{osp.splitext(osp.basename(args.config))[0]}_{time_id}')
...
########################################################################################
    ...
    model.backbone.init_weights() # load clip image encoder weights

    if hasattr(model, 'text_encoder'):
        model.text_encoder.init_weights()  # load clip text encoder weights

    if distributed:
        model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model)
        # model.backbone = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model.backbone)
        # model.bbox_head = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model.bbox_head)
        # if model.neck is not None:
        #     model.neck = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model.neck)
    ...
    # config Ranger21 optimizer
    if cfg.optimizer.type == 'Ranger21':
        cfg.optimizer.num_epochs = cfg.total_epochs
        cfg.optimizer.num_batches_per_epoch = int(len(datasets[0]) / cfg.data.samples_per_gpu /8)

    # replace original single_gpu_test with dynamic_single_gpu_test
    # if cfg.data.val['type'] == 'PostcfgUniformConcatDataset':
    #     eval_cfg = cfg.get('evaluation', {})
    #     if distributed:
    #         eval_cfg['test_fn'] = dynamic_multi_gpu_test
    #     else:
    #         eval_cfg['test_fn'] = dynamic_single_gpu_test
